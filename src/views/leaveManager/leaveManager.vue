	...
	<section>
		...
		<el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
			<el-form :inline="true" :model="filters">
				...
				<el-form-item>
					<el-button @click="handleEdit" icon="el-icon-plus">出差申请</el-button>
				</el-form-item>
			</el-form>
		</el-col>
		...
		<!--出差申请界面-->
		<el-dialog title="出差申请" :visible.sync="traveFormVisible" width="30%" append-to-body :close-on-click-modal="false">
			<trave-application @closeDialog="closeDialog"></trave-application>
		</el-dialog>
			...
			<el-tabs>
				<el-row style="padding:0 40px;">
					<el-col :span="24">
						...
						<dl class="dllist boderBottom">
							<dt class="fl">出差类型:</dt>
							<dd v-if="deptData.type">{{ deptData.type == 1 ? '会议' : (deptData.type == 2 ? '培训' :'外派') }}</dd>
						</dl>
						<dl class="dllist boderBottom">
							<dt class="fl">出差开始时间:</dt>
							<dd v-if="deptData.startTime">{{ deptData.startTime }}</dd>
						</dl>
						<dl class="dllist boderBottom">
							<dt class="fl">出差结束时间:</dt>
...
<script>
	...
	import travelApplication from '../../components/leaveManager/travelApplication.vue'

	export default {
		components: {
			'trave-application': travelApplication
		},
		data() {
			return {
				filters: {
