    ...
    <div class="store">
        ...
        <div class="search-header">
            <el-row >
                ...
                <el-col style="height: 50px;" class="VCenter" :span="5">
                    <div class="floatLeft" style="margin-left: 34px;width: 45%;">班级职务：</div>
                    <el-input v-model="searchInfo.dutyName" style="width: auto;" class="floatLeft" placeholder="请输入内容"></el-input>
                </el-col>
                <el-col style="height: 50px;" class="VCenter" :span="5">
                    <div class="floatLeft" style="margin-left: 34px;width: 45%;">学生姓名：</div>
        ...
        >
            <el-form label-width="80px">
                <el-form-item label="学生姓名">
                    <el-select :clearable="true" v-model="editInfo.studentId" style="width: 70%;" class="floatLeft" placeholder="请选择">
                        <el-option
        ...
        >
            <el-form label-width="80px">
                <el-form-item label="班级名称">
                    <el-select :clearable="true"  @change="seletedClassChange" v-model="addInfo.classId" style="width: 70%;" class="floatLeft" placeholder="请选择">
                        <el-option
                ...
                <el-form-item label="学生职务">
                    <el-select :clearable="true" v-model="addInfo.dutyName" style="width: 70%;" class="floatLeft" placeholder="请选择">
                        <el-option
                ...
                <el-form-item label="学生姓名">
                    <el-select :clearable="true" v-model="addInfo.studentId" style="width: 70%;" class="floatLeft" placeholder="请选择">
                        <el-option
                                v-for="item in studentOptions"
                                :key="item.studentId"
                                :label="item.studentName"
            ...
            <div class="seletePrompt">注意选择学生前应先选好班级!!!</div>
...
<script>
    ...
    function requestList(submitData){
        ...
        httpRequest.get("/edu-system/class/student/getAllDuty",submitData,data=>{
    ...
    function handleCanNotParam(submitData){
        ...
        if(this.searchInfo.dutyName){
            submitData["dutyName"]=this.searchInfo.dutyName;
        }
    ...
    //获取选择的班级的学生
    function getClassStudents(url){
        httpRequest.get(url,"",data=>{
            if(data.data.meta.success){
                this.studentOptions=data.data.data;
                this.addInfo.studentId="";
            }else{
                this.$message.error('获取选择的班级的学生失败，请重试');
            }
        });
    }
    export default {
        ...
        mounted(){
            ...
            httpRequest.get("/edu-system/code/getTypeCodes",submitDutyData,data=>{
                if(data.data.meta.success){
                    this.dutyOptions=data.data.data;
                }else{
                    this.$message.error('获取职务名称失败，请重试');
                }
            });
        },
        data(){
        ...
        methods: {
            ...
            seletedClassChange(val){//选择了班级 ，班级变化 获取对应班级的学生
                //获取该班级的所有学生
                var url="/edu-system/class/getStudentByClass/"+val;
                getClassStudents.call(this,url);
            },
            search(){
                var submitData={
                    currentPage : 1,
            ...
            add(){
                if(this.addInfo.classId && this.addInfo.studentId && this.addInfo.dutyName){
                    var submitData={
                        classId:this.addInfo.classId,
                        studentId:this.addInfo.studentId,
                        dutyName:this.addInfo.dutyName
                    };
                    if(this.addInfo.remark){
                        submitData["description"]=this.addInfo.remark;
                    }
                    //发送请求
                    httpRequest.post("/edu-system/class/student/duty",submitData,data=>{
                        if(data.data.meta.success){
                            var submitData={
            ...
            showEdit(index,row){
                ...
                //获取班级的学生
                var url="/edu-system/class/getStudentByClass/"+row.classId;
                getClassStudents.call(this,url);
            },
            edit(){
                if(this.editInfo.studentId){
                    var submitData={
                    ...
                    //发送请求
                    httpRequest.put("/edu-system/class/student/duty",submitData,data=>{
                        if(data.data.meta.success){
                            this.editDialog=false;//隐藏编辑对话框
                            //同步页面更改
                            this.listInfo[this.editObject.index]["studentName"]=data.data.data.studentName;
            ...
            delete_(){
                var url="/edu-system/class/student/duty/"+this.deleteObject.row.id;
