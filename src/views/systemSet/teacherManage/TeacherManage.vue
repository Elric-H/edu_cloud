    ...
    <div class="store">
        ...
        <div class="search-header">
            <el-row >
                ...
                <el-col style="height: 50px;" class="VCenter" :span="5">
                    ...
                    <el-input v-model="searchInfo.teacherName" style="width: auto;" class="floatLeft" placeholder="请输入内容"></el-input>
                </el-col>
                <el-col style="height: 50px;" class="VCenter" :span="5">
                    <div class="floatLeft" style="margin-left: 34px;width: 45%;">所教科目：</div>
                    <el-input v-model="searchInfo.subjectName" style="width: auto;" class="floatLeft" placeholder="请输入内容"></el-input>
                </el-col>
        ...
        >
            <el-form label-width="80px">
                <el-form-item label="老师姓名">
                    <el-select :clearable="true" v-model="editInfo.teacherId" style="width: 70%;" class="floatLeft" placeholder="请选择">
                        <el-option
        ...
        >
            <el-form label-width="80px">
                ...
                <el-form-item label="老师姓名">
                    <el-select :clearable="true" v-model="addInfo.teacherId" style="width: 70%;" class="floatLeft" placeholder="请选择">
                        <el-option
                                v-for="item in teacherOptions"
                                :key="item.id"
                                :label="item.name"
...
<script>
    ...
    function requestList(submitData){
        ...
        httpRequest.get("/edu-system/class/getAllClassTeacher",submitData,data=>{
    ...
    function handleCanNotParam(submitData){
        ...
        if(this.searchInfo.teacherName){
            submitData["teacherName"]=this.searchInfo.teacherName;
        }
        if(this.searchInfo.subjectName){
            submitData["subjectName"]=this.searchInfo.subjectName;
        }
    ...
    export default {
        ...
        methods: {
            ...
            add(){
                if(this.addInfo.classId && this.addInfo.teacherId && this.addInfo.subjectName){
                    var submitData={
                        classId:this.addInfo.classId,
                        teacherId:this.addInfo.teacherId,
                        subjectName:this.addInfo.subjectName
                    };
                    //发送请求
                    httpRequest.post("/edu-system/class/teacher",submitData,data=>{
                        if(data.data.meta.success){
                            var submitData={
            ...
            edit(){
                if(this.editInfo.teacherId){
                    var submitData={
                        id:this.editObject.row.id,
                        teacherId:this.editInfo.teacherId,
                    };
                    //发送请求
                    httpRequest.put("/edu-system/class/teacher",submitData,data=>{
                        if(data.data.meta.success){
                            this.editDialog=false;//隐藏编辑对话框
                            //同步页面更改
                            this.listInfo[this.editObject.index]["teacherName"]=data.data.data.teacherName;
                            this.$message({
                                message: '编辑成功',
                                type: 'success'
                            });
            ...
            delete_(){
                var url="/edu-system/class/teacher/"+this.deleteObject.row.id;
